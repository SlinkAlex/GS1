...
class EtiquetaPdf < Prawn::Document
  ...
  def initialize(empresa, telefono, municipio_ean)
    ...
    font("#{Rails.root}/fonts/arial.ttf", :size => 8) do
      ...
      text_box("#{empresa.direccion_ean.strip}", options={:at => [40,75], :width  =>225})
      ...
      draw_text "Municipio: #{municipio_ean.try(:nombre)}", :at => [0,21]
